...
export function mergeGlobsAndExcludes(defaults, userConfig, cliConfig) {
  ...
  if (hasProvidedCliGlobs) {
    defaults.globs = defaults.globs.filter(
      (glob) => glob !== "**/*.{js,ts,tsx}",
    );
  }
...
export async function getUserConfig(configPath, cwd) {
  console.log("cli-helpers.js getUserConfig: configPath =", configPath);
  console.log("cli-helpers.js getUserConfig: cwd =", cwd);
  let userConfig = {};
  try {
    userConfig = await readConfig(
  ...
  console.log("cli-helpers.js getUserConfg: userConfig =", userConfig);
...
export function getCliConfig(argv) {
  const CLI_OPTIONS = [
    ...
    { name: "catalyst-major-2", type: Boolean },
    {
      ...
      type: String,
      description: "JSON string with oxc-resolver options",
    },
  ];

  return commandLineArgs(CLI_OPTIONS, { argv });
}
...
export async function addFrameworkPlugins(mergedOptions) {
  ...
  if (mergedOptions?.litelement) {
    const { litPlugin } =
      await import("../features/framework-plugins/lit/lit.js");
    plugins = [...(litPlugin() || [])];
  }

  if (mergedOptions?.fast) {
    const { fastPlugin } =
      await import("../features/framework-plugins/fast/fast.js");
    plugins = [...(fastPlugin() || [])];
  }

  if (mergedOptions?.stencil) {
    const { stencilPlugin } =
      await import("../features/framework-plugins/stencil/stencil.js");
    plugins.push(stencilPlugin());
  }

  if (mergedOptions?.catalyst) {
    const { catalystPlugin } =
      await import("../features/framework-plugins/catalyst/catalyst.js");
    plugins = [...(catalystPlugin() || [])];
  }

  if (mergedOptions?.["catalyst-major-2"]) {
    const { catalystPlugin2 } =
      await import("../features/framework-plugins/catalyst-major-2/catalyst.js");
